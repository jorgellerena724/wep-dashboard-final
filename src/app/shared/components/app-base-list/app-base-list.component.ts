...
export class BaseListComponent<T = HomeData> {
  ...
  protected langChanged = toSignal(this.transloco.langChanges$, {
    initialValue: this.transloco.getActiveLang(),
  });

  // Computed signals con traducciones reactivas
  protected columns = computed<Column[]>(() => {
    // Forzar recalculo cuando cambie el idioma
    this.langChanged();
  ...
  protected headerActions = computed<TableAction[]>(() => {
    // Forzar recalculo cuando cambie el idioma
    this.langChanged();
  ...
  protected rowActions = computed<RowAction[]>(() => {
    // Forzar recalculo cuando cambie el idioma
    this.langChanged();
  ...
  protected processData(items: T[]): void {
    ...
    // Solo procesar status si la configuración lo habilita
    const processed = items.map((item: any) => {
      const processedItem = { ...item };

      // Agregar statusToShow solo si status está habilitado
      if (config.status?.enabled && item.status !== undefined) {
        processedItem.statusToShow = item.status
  ...
  protected loadMediaFiles(items: any[]): void {
    ...
    // Solo cargar media si está configurado
  ...
  protected async handleDelete(row: any): Promise<void> {
    ...
    if (config?.actions?.delete?.confirmDialog) {
      const confirmed = await this.confirmDialogService.confirm({
        title: this.transloco.translate('common.delete.title'),
        message: this.transloco.translate('common.delete.message'),
        confirmLabel: this.transloco.translate('common.delete.confirm'),
        cancelLabel: this.transloco.translate('common.delete.cancel'),
      });
    ...
    config.service
      ...
      .subscribe({
        ...
        error: (error: any) => {
          // Intentar usar el manejador de errores personalizado primero
          const customErrorHandler = config.actions?.delete?.customErrorHandler;
          if (customErrorHandler) {
            const customMessage = customErrorHandler(error, this.transloco);
            if (customMessage) {
              this.notificationSrv.addNotification(customMessage, 'error');
              this.loading.set(false);
              return;
            }
          }

          // Si no hay manejador personalizado o no retornó mensaje, usar el manejo por defecto
  ...
  protected handleToggleStatus(row: any): void {
    ...
    // Verificar que status esté habilitado
    if (!config.status?.enabled) return;

    // Validar mínimo de items activos si está configurado
