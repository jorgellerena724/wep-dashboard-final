...
export class ManagerService {
  ...
  get(): Observable<HomeData[]> {
    ...
    return this.http
      .get<HomeData[]>(this.apiUrl + `manager/?no-cache=${timestamp}`)
      .pipe(
        tap((data) => {
  ...
  post(data: any): Observable<any[]> {
    return this.http
      .post<any[]>(this.apiUrl + 'manager/', data)
      .pipe(tap(() => this.invalidateCache()));
  }

  patch(formData: FormData, id: number): Observable<any> {
    return this.http
      .patch(`${this.apiUrl}manager/${id}`, formData)
      .pipe(tap(() => this.invalidateCache()));
  }

  delete(id: number): Observable<any> {
    return this.http
      .delete<any>(`${this.apiUrl}manager/${id}`, {
        body: { id: id },
      })
      .pipe(tap(() => this.invalidateCache()));
  }
